...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-a" | "--aliases" => {
                let if_exists = args.iter().any(|a| a == "--if-exists");
                print_aliases(if_exists);
                std::process::exit(0);
            }
            "--if-exists" => {
                if !args.iter().any(|a| a == "-a" || a == "--aliases") {
                    eprintln!("rgc: --if-exists requires -a/--aliases");
                    std::process::exit(1);
                }
                // Handled in the -a/--aliases branch above; just skip.
            }
            "--update" => {
                update::run();
                std::process::exit(0);
...
{b}{y}Options:{r}
  ...
  {c}-a{r}, {c}--aliases{r}              Generate shell aliases for .bashrc/.zshrc
      {c}--if-exists{r}            With {c}-a{r}: emit aliases only for commands found in PATH
...
fn print_aliases(if_exists: bool) {
    ...
    if if_exists {
        let found = commands_in_path(&aliases);
        aliases.retain(|cmd| found.contains(*cmd));
    }

    let gen_line = if if_exists {
        "# Generated by: rgc --aliases --if-exists\n"
    } else {
        "# Generated by: rgc --aliases\n"
    };
...
/// Scan each $PATH directory in parallel, returning the subset of `needed`
/// command names that appear as a file/symlink inside any of them.
///
/// One `read_dir` per PATH entry on a dedicated thread; much faster than
/// stat-per-(cmd,dir) on WSL where /mnt/c mounts make individual stats slow.
fn commands_in_path(needed: &std::collections::BTreeSet<&str>) -> std::collections::HashSet<String> {
    ...
    let needed_set: Arc<HashSet<String>> = Arc::new(needed.iter().map(|s| s.to_string()).collect());
    let found: Arc<Mutex<HashSet<String>>> = Arc::new(Mutex::new(HashSet::new()));

    // Dedupe PATH entries — many setups list the same dir twice.
    let mut seen_dirs = HashSet::new();
    let dirs: Vec<_> = std::env::split_paths(&paths)
    ...
    let handles: Vec<_> = dirs
        .into_iter()
        .map(|dir| {
            let needed = Arc::clone(&needed_set);
            ...
            thread::spawn(move || {
                ...
                for entry in entries.flatten() {
                    // Use file_type() (typically from getdents64, no extra stat)
                    // to skip directories; symlinks count (many bins are symlinks).
    ...
    Arc::try_unwrap(found).ok().unwrap().into_inner().unwrap()
}
